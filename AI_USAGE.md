...
- **Правки дизайна / UX после ручного ревью кандидата:**
  - даты в карточке: вместо смеси русской даты и английского AM/PM — строгий формат `ДД.ММ.ГГГГ, ЧЧ:мм`;
  - пагинация списка: на мобилке только sticky снизу (верхнюю убрали), на desktop обе панели справа; в `page_size` добавлен `1`;
  - модалка фильтров на мобилке: AI сначала задал радиус через устаревший `styles.content` (в Ant Design 6 не применяется) и боковой drawer — по замечанию переделано на bottom sheet + `styles.section` / скругление верхних углов;
  - подписи точек маршрута `load` / `unload` на деталке — заменены на «Погрузка» / «Выгрузка»;
  - выравнивание карточек деталки (равная высота, читаемый блок маршрута без узкой колонки).
...
- На мобилке листинг вместо широких таблиц: компактные карточки с минимальным описанием + раскрытие (accordion) или модалка по тапу со доп. полями — без перехода на деталку только ради просмотра краткой сводки.
